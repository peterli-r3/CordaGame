...
class Controller(rpc: NodeRPCConnection) {
    ...
    @PostMapping(value = ["startGame/{whoAmI}/{competeWith}"])
    ...
    @PostMapping(value = ["endGame/{whoAmI}/{competeWith}"])
    fun endGame(@PathVariable whoAmI:String,
                   @PathVariable competeWith:String):ResponseEntity<String> {
        return try {
            val gameId = proxy.startFlow(::myGame,whoAmI).returnValue.get().linearId
            proxy.startFlow(::EndGameFlow, gameId, whoAmI,competeWith).returnValue.get()
            ResponseEntity.status(HttpStatus.CREATED).body("Game Over")
        } catch (e: Exception) {
            ResponseEntity.status(HttpStatus.BAD_REQUEST).body(e.message)
        }
    }
    ...
    /**
     * Returns the if the game is over.
     */
    @PostMapping(value = ["gameInSession/{whoAmI}"])
    fun gameover(@PathVariable whoAmI:String): ResponseEntity<String>{
        return try {
            if(isGameOver(whoAmI)){
                ResponseEntity.status(HttpStatus.CREATED).body("True")
            }else{
                ResponseEntity.status(HttpStatus.CREATED).body("False")
            }
        } catch (e: Exception) {
            ResponseEntity.status(HttpStatus.BAD_REQUEST).body(e.message)
        }
